...
const loadImage = () => {
    ...
    previewImg.addEventListener("load", () =>{
        resetFilterBtn.click(); //reset filters for new images
...
rotateOptions.forEach(option => {
    option.addEventListener("click", () => {     //adding click event listener to all rotate buttons
        if(option.id === "left") {
            ...
            flipHorizontal = flipHorizontal === 1 ? -1 :1; // if clicked btn is left rotate, decrement rotate value by -90
        }
        else {
            flipVertical = flipVertical === 1 ? -1 :1; // if clicked btn is left rotate, decrement rotate value by -90
        }
...
const resetFilter =  () => {
    // resetting all variable value to its default value
     ...
     filterOptions[0].click(); //clicking brightness btn, so the brigtness selected by default
     applyFilters();
}
...
const saveImage = () => {
     ...
     //apllying user selected filters to canvas filter
